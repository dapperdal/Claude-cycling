...
WORKOUT_LIBRARY: Dict[str, dict] = {
    "zone2": {
        ...
        "description": "Base building, fat burning, recovery (2-3x/week)",
        "frequency_hint": "Do this 2-3x per week for aerobic base",
        "duration_minutes": 60,
        "intensity": "Low"
    },
    "vo2max": {
...
class WorkoutConfig:
    ...
    # Zone 2 is typically 56-75% of FTP, we'll use 70% as target (upper Z2)
    @property
    def zone2_power(self) -> int:
        return int(self.ftp * 0.70)

    @property
    def zone2_low(self) -> int:
        return int(self.ftp * 0.56)

    @property
    def zone2_high(self) -> int:
        return int(self.ftp * 0.75)
...
class WorkoutManager:
    ...
    def __init__(self, ftp: int = 215):
        self.config = WorkoutConfig(ftp=ftp)
